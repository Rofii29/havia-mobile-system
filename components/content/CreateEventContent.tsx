  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300 pb-32">
      <div className="space-y-1.5">
        ...
        <input type="text" value={newEvent.title} onChange={(e) => setNewEvent({ ...newEvent, title: e.target.value })} placeholder="Nama pertemuan/kegiatan..." style={{ backgroundColor: colors.card, borderColor: colors.border }} className={inputClass} />
      </div>

      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1.5">
          <label className="text-[10px] font-bold text-neutral-500 ml-1 uppercase tracking-widest">Tanggal</label>
        ...
        <div className="space-y-1.5">
          <label className="text-[10px] font-bold text-neutral-500 ml-1 uppercase tracking-widest">Waktu</label>
      ...
      <div className="space-y-1.5">
        ...
        <input type="text" value={newEvent.location} onChange={(e) => setNewEvent({ ...newEvent, location: e.target.value })} placeholder="Ruang Rapat / Google Meet..." style={{ backgroundColor: colors.card, borderColor: colors.border }} className={inputClass} />
      </div>

      <div className="space-y-1.5">
        <label className="text-[10px] font-bold text-neutral-500 ml-1 uppercase tracking-widest">Pilih Warna Label</label>
        <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
          {['#C69C3D', '#F43F5E', '#3B82F6', '#10B981', '#8B5CF6'].map((c) => (
            <div key={c} onClick={() => setNewEvent({ ...newEvent, color: c })} className={`w-10 h-10 rounded-full cursor-pointer shrink-0 border-2 transition-all ${newEvent.color === c ? 'border-white scale-110' : 'border-transparent opacity-50'}`} style={{ backgroundColor: c }} />
          ))}
        </div>
      </div>
